...
bool App::OnInit()
{
    // オブジェクト（頂点・定数バッファ・PSO 等）の初期化を Object に委譲
    if (!m_Object.Initialize(m_pDevice.Get(), m_Width, m_Height, FrameCount))
    { return false; }

    // ビューポート / シザー等は Object が設定するのでここでは追加処理なし
    return true;
}
...
void App::OnTerm()
{
    ...
    // App 側で保持している頂点バッファや PSO は削除済のためここではそれらを操作しない
}
...
void App::Render()
{
    // 更新処理.
    {
        // m_RotateAngle は App クラスに残していないためローカル static で保持する
        static float rotateAngle = 0.0f;
        rotateAngle += 0.025f;
        // Object に更新を任せる（Object::Update は内部で定数バッファを更新）
        m_Object.Update(m_FrameIndex, rotateAngle);
    }
    ...
    // 描画（Object を 3x3 タイルで描画して地面を作成）
    {
        const int tiles = 3;
        const float quadSize = 2.0f; // Quad の幅は -1..1 なので 2.0f
        const float spacing = quadSize;
        const float offset = ((tiles - 1) * spacing) * 0.5f; // 中心オフセット

        for (int r = 0; r < tiles; ++r)
        {
            for (int c = 0; c < tiles; ++c)
            {
                float x = c * spacing - offset; // 中心が 0 になるように計算
                float z = r * spacing - offset;
                // ワールド行列 = 回転 * 平行移動（回転は同じ rotateAngle を使用）
                auto translation = DirectX::XMMatrixTranslation(x, -1.0f, z);
                auto rotation = DirectX::XMMatrixRotationY(rotateAngle);
                DirectX::XMMATRIX world = rotation * translation;
                m_Object.DrawAt(m_pCmdList.Get(), m_FrameIndex, world);
            }
        }
    }
