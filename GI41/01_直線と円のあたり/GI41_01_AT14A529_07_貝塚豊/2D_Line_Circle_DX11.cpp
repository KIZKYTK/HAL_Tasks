...
bool HitCheck()
{
    ...
    // 線分ベクトルを求める
    //=========================================================
    float lineX = (float)(g_Line.pos_x1 - g_Line.pos_x0);
    float lineY = (float)(g_Line.pos_y1 - g_Line.pos_y0);

    //=========================================================
    // 線分開始点 → 円中心ベクトル
    //=========================================================
    float circleX = (float)(g_Circle.pos_x - g_Line.pos_x0);
    float circleY = (float)(g_Circle.pos_y - g_Line.pos_y0);

    //=========================================================
    // 内積から射影値 t を求める
    //=========================================================
    float lineLengthSq = lineX * lineX + lineY * lineY;
    ...
    if (lineLengthSq <= 0.0f)
    {
        ...
        float dx = (float)(g_Circle.pos_x - g_Vx);
        float dy = (float)(g_Circle.pos_y - g_Vy);

        return (dx * dx + dy * dy) <=
            (float)(g_Circle.radius * g_Circle.radius);
    }

    float t =
        (circleX * lineX + circleY * lineY) / lineLengthSq;
    ...
    //=========================================================
    float nearestX =
        (float)g_Line.pos_x0 + lineX * t;

    float nearestY =
        (float)g_Line.pos_y0 + lineY * t;
    ...
    // 最近接点 → 円中心距離
    //=========================================================
    float dx = (float)g_Circle.pos_x - nearestX;
    float dy = (float)g_Circle.pos_y - nearestY;
